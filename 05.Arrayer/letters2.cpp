...
{
    int sprak1, sprak2;
    double skillnad, sum = 0.0;

    cout << "Vilka två språk vill du jämföra [(1)Eng,(2)Fra,(3)Sve,(4)Tys]?" << endl;
    cin >> sprak1 >> sprak2;

    if((sprak1>=1 && sprak1<=4) && (sprak2>=1 && sprak2<=4))
    {
        for(int i=0; i<ANTAL_BOKSTAVER; i++)
        {
            skillnad=FREKV[sprak1-1][i]-FREKV[sprak2-1][i];
            sum = sum + skillnad*skillnad;
        }
    cout << "Likheten = " << sum << endl;
    }
